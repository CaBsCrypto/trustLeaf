...
export async function POST(req: NextRequest) {
  try {
    ...
    const type = (body.type ?? "paciente").trim();
    ...
    if (!apiKey) {
      ...
      console.log("[waitlist] DEMO MODE — no RESEND_API_KEY. Email:", email, "Name:", name, "Type:", type);
    ...
    const notifyDetails = [
      ...
      name ? `<strong>Nombre:</strong> ${name}` : null,
      `<strong>Tipo:</strong> ${type}`,
      specialty ? `<strong>Especialidad:</strong> ${specialty}` : null,
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `🌿 Nuevo ${type} en waitlist: ${name || email}`,
        html: `<p>Nuevo registro en el waitlist de TrustLeaf:</p><p>${notifyDetails}</p>`,
      }),
    });
